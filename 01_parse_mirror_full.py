#!/usr/bin/env python3
"""
01_parse_mirror_full.py — Stage 1: Extract raw facts from HTML mirror (full variant)

This script:
- Reads local offline mirror under ./mirror_full (or --mirror-dir)
- Extracts raw event data from HTML (no semantic cleaning)
- Outputs: out/stage1_raw_events__{tag}.csv (tag derived from mirror dir name)

Input: {mirror_dir}/www.footbag.org/events/show/*/index.html
Output: out/stage1_raw_events__{tag}.csv, stage1_qc_summary__{tag}.json, stage1_qc_issues__{tag}.jsonl
"""
...
def print_verification_stats(records: list[dict]) -> None:
    ...
    # "Events with results" gate (simple + stable):
    # count rows where results_block_raw is non-empty after extraction.
    events_with_results = sum(1 for r in records if r.get("results_block_raw"))
    pct = (events_with_results / total) * 100 if total else 0.0
    print(f"Events with results_block_raw: {events_with_results}/{total} ({pct:5.1f}%)")
...
def _mirror_tag_from_path(mirror_dir: Path) -> str:
    """
    Derive a stable tag for filenames from the mirror directory name.
    Examples:
      ~/.../mirror_full -> mirror_full
      ./mirror         -> mirror
    """
    name = (mirror_dir.name or "mirror").strip()
    name = re.sub(r"[^A-Za-z0-9._-]+", "_", name)
    return name or "mirror"


def _default_mirror_dir(repo_dir: Path) -> Path:
    """Pick a sensible default mirror directory, preferring mirror_full."""
    candidates = [
        repo_dir / "mirror_full",
        repo_dir / "mirror",
    ]
    for c in candidates:
        if c.exists():
            return c
    # Fall back to repo_dir/mirror_full even if missing; find_events_show_dir() will error clearly.
    return repo_dir / "mirror_full"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Stage 1: parse HTML mirror into raw events CSV")
    parser.add_argument(
        "--mirror-dir",
        type=Path,
        default=None,
        help="Path to mirror root (contains www.footbag.org/...). Defaults to mirror_full if present.",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=(repo_dir / "out"),
        help="Output directory (default: ./out)",
    )
    args = parser.parse_args()

    mirror_dir = (args.mirror_dir or _default_mirror_dir(repo_dir)).expanduser().resolve()
    out_dir = args.out_dir.expanduser().resolve()
    tag = _mirror_tag_from_path(mirror_dir)

    # Sane, non-overlapping outputs for multiple mirrors
    out_csv = out_dir / f"stage1_raw_events__{tag}.csv"
    ...
    # Write QC outputs with mirror-specific suffix to avoid overwriting
    summary_path = out_dir / f"stage1_qc_summary__{tag}.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(qc_summary, f, indent=2, ensure_ascii=False)
    print(f"Wrote: {summary_path}")

    issues_path = out_dir / f"stage1_qc_issues__{tag}.jsonl"
    with open(issues_path, "w", encoding="utf-8") as f:
        for issue in qc_issues:
            f.write(json.dumps(issue, ensure_ascii=False) + "\n")
    print(f"Wrote: {issues_path} ({len(qc_issues)} issues)")
