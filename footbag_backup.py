...
class MirrorState:
    def __init__(self):
        ...
        self.stats.setdefault('videos_skipped', 0)
        self.stats.setdefault('events_with_results', 0)
...
def filter_results(url: str) -> bool:
    """Return True if this URL should be visited. Allow all in-scope URLs except video files."""
    if is_video_file(url):
        return False
    return is_in_scope(url)
...
def print_stats():
    ...
    print(f"Videos skipped (filtered): {s.get('videos_skipped', 0):,}")
    print(f"Events with results: {s.get('events_with_results', 0):,}")
    print("========================\n")
                ...
                if tid_match:
                    ...
                    try:
                        ...
                        if reg_resp.status_code == 200:
                            ...
                            if has_members:
                                mirror_state.stats.setdefault('events_with_results', 0)
                                mirror_state.stats['events_with_results'] += 1
