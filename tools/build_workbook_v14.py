...
Changes from v12:
  - Year sheets: each event column has a workbook-level defined name
    ``event_<event_id>`` anchored to the event title cell (row 1).
  - EVENT INDEX: hyperlinks now target the defined name anchor
    (#event_<event_id>) instead of a raw column reference.
  - Post-build verification: every EVENT INDEX event_id is checked against
    the defined names dictionary; any missing anchors are reported.
...
OUTPUT_PATH      = os.path.join(BASE_DIR, "out", "Footbag_Results_Community_v14_Canonical.xlsx")

PF_CSV           = os.path.join(BASE_DIR, "out", "canonical_pf.csv")
...
CANONICAL_ALL_EVENTS_CSV = os.path.join(BASE_DIR, "out", "canonical_all", "events_all.csv")

# Records outputs (early_data/out/records/)
_RECORDS_DIR         = os.path.join(BASE_DIR, "early_data", "out", "records")
RECORDS_SUMMARY_CSV  = os.path.join(_RECORDS_DIR, "records_summary.csv")
RECORDS_LEADER_CSV   = os.path.join(_RECORDS_DIR, "records_leaderboard_by_trick.csv")
RECORDS_QUEUE_CSV    = os.path.join(_RECORDS_DIR, "records_review_queue.csv")
RECORDS_UNRES_CSV    = os.path.join(_RECORDS_DIR, "records_unresolved_players.csv")
...
def build_readme(wb: Workbook, quarantine_count: int = 0,
    ...
    for name, desc in [
        ...
        ("EXCLUDED EVENTS",           "Events in the archive that are NOT in the published results, with reasons"),
        ("CORRECTIONS",               "Verified historical corrections (1987/1995/1996 Worlds data)"),
        ("STATISTICS",                "Career podiums, event wins, events competed, career spans, events by year"),
        ("EVENT INDEX",               "All documented events — published (green), excluded (gray), quarantined (red)"),
        ("PLAYER SUMMARY",            "Competition history per player — wins, podiums, placements, events"),
        ("CONSECUTIVE RECORDS",       "Documented consecutives world records (competition results)"),
        ("RECORDS SUMMARY",           "Consecutives trick-specific world records — one row per trick"),
        ("RECORDS LEADERBOARD",       "Full leaderboard for each consecutives trick"),
        ("RECORDS REVIEW QUEUE",      "Consecutives records flagged for manual review"),
    ...
    for note in [
        ...
        "Pre-1997 data (37 events) is sourced from the Footbag.org archive and Footbag World magazine.",
        "  • 1980–1991: major Worlds and championship events; results are partial (top finishers only).",
        "  • 1992–1996: one event each year (Worlds / major championship).",
        "  • 1988: no coverage (source data removed — mislabeled; actual 1987 data).",
...
def build_data_notes(wb: Workbook, fffd_count: int = 0, quarantine_count: int = 0,
                     published_count: int = 0, excluded_count: int = 0) -> None:
    ...
    kv("1992–1996", "Sparse — one event per year from the Footbag.org archive and Footbag World magazine. "
                    "Each year has exactly one documented event (Worlds or major championship).")
    ...
    section("Event Publication Rules")
    body("Not every documented event appears in the year sheets. "
         "An event must have at least one division with 3 or more recorded placements to be published. "
         f"Of {published_count + excluded_count} documented events, "
         f"{published_count} are published and {excluded_count} are excluded. "
         "Excluded events are listed in the EXCLUDED EVENTS sheet.")
    kv("no_results",        "Events on Footbag.org that posted no competitive results (e.g. registration "
                             "only, social events, or events where results were never uploaded). "
                             "30 events fall into this category.")
    kv("all_sparse",        "Events where every division has fewer than 3 recorded placements — "
                             "typically fragmentary historical records where only 1-2 names survive. "
                             "These are documented but cannot contribute to player statistics.")
    kv("historical removed","Two events were removed after verification: the 1988 WFA Worlds (data "
                             "was actually from 1987 — mislabeled) and the 1995 IFAB Worlds (a "
                             "lesser-known parallel sanctioning body; replaced by the corrected 1987 entry).")
    ...
    if os.path.exists(EVENTS_CSV):
        for r in load_csv(EVENTS_CSV):
            # canonical events.csv uses legacy_event_id as the numeric event_id
...
CANONICAL_EVENTS_CSV = os.path.join(BASE_DIR, "out", "release_publication", "events.csv")
...
def load_events_for_year_sheets() -> dict:
    """
    Returns dict: event_id (slug) → event metadata dict.
    Uses out/release_publication/events.csv (merged PRE1997 + POST1997).
    """
    result: dict = {}
    ...
    for r in load_csv(CANONICAL_EVENTS_CSV):
        # release_publication uses "event_id" (slug) as the primary key
        ...
        # Fall back to combined "location" field (populated for pre-1997 events)
        ...
        result[eid] = {
            ...
            "data_source": r.get("data_source", "").strip(),
        }
    return result
...
def build_event_index(wb: Workbook,
    ...
    cols   = ["Event ID", "Year", "Event Name", "City / Region", "Country",
              ...
              "Source", "Status", "Notes"]
    widths = [14, 6, 48, 28, 18, 13, 11, 10, 12, 10, 16, 48]
    ...
    # Include ALL documented events (from release_publication + excluded from canonical_all)
    # Merge events dict (from load_events_for_year_sheets) with excluded events
    all_eids_set = set(events.keys())

    # Load canonical_all events to find excluded ones not already in events dict
    if os.path.exists(CANONICAL_ALL_EVENTS_CSV):
        for r in load_csv(CANONICAL_ALL_EVENTS_CSV):
            eid = r.get("event_id", "").strip()
            if eid and eid not in events:
                # Build a minimal metadata entry for excluded event
                city    = r.get("city", "").strip()
                region  = r.get("region", "").strip()
                country = r.get("country", "").strip()
                if city and country in ("United States", "Canada") and region:
                ...
                events[eid] = {
                    ...
                    "event_name": r.get("event_name", "").strip(),
                    "year":       r.get("year", "").strip(),
                    "date":       start or r.get("year", "").strip(),
                    "location":   loc,
                    "country":    country,
                    "host_club":  r.get("host_club", "").strip(),
                    "event_type": r.get("event_type", "").strip(),
    ...
    # Color fills for event index status
    _IDX_FILL_EXCL = PatternFill("solid", fgColor="F5F5F5")  # gray for excluded/no-results

    n_published = n_excluded = 0
    for row_idx, eid in enumerate(all_eids, start=2):
        ...
        status_raw   = ev.get("status_raw", "")
        is_published = n_p > 0 and not is_quar

        if is_quar:
            status = "QUARANTINED"
            notes  = "Excluded — ambiguous structure prevents deterministic parsing"
        elif n_p == 0 and status_raw == "no_results":
            status = "NO_RESULTS"
            notes  = "No competitive results available in source"
        elif n_p == 0:
            status = "EXCLUDED"
            notes  = "All divisions have fewer than 3 placements"
        elif issue:
        ...
        # City display: use location string first part or country
        ...
        for c, val in enumerate(values, start=1):
            ...
            if c == 12 and val:   # notes italic (col 12 now)
        ...
        # Row fill: red=quarantined, green=published, gray=excluded
        if is_quar:
            row_fill = _IDX_FILL_QUAR
        elif is_published and status == "PUBLISHED":
            row_fill = _IDX_FILL_OK
        else:
            row_fill = _IDX_FILL_EXCL
        for c in range(1, len(cols) + 1):
            ws.cell(row=row_idx, column=c).fill = row_fill

    print(f"  EVENT INDEX: {len(all_eids)} rows ({n_published} published, {n_excluded} excluded/quarantined)")


# ── EXCLUDED EVENTS sheet ─────────────────────────────────────────────────────

def build_excluded_events_sheet(wb: Workbook, published_ids: set,
                                 placements_by_event: dict,
                                 quarantine_set: set) -> int:
    """
    Add EXCLUDED EVENTS sheet listing all events NOT in the published results.
    published_ids: set of event_ids that appear in the PF (canonical_pf.csv).
    Returns the count of excluded events written.
    """
    if "EXCLUDED EVENTS" in wb.sheetnames:
        del wb["EXCLUDED EVENTS"]
    # Position after DATA NOTES
    if "DATA NOTES" in wb.sheetnames:
        idx = wb.sheetnames.index("DATA NOTES") + 1
    else:
        idx = 1
    ws = wb.create_sheet("EXCLUDED EVENTS", idx)

    # Load all events from canonical_all (full unfiltered set)
    all_events: dict = {}
    if os.path.exists(CANONICAL_ALL_EVENTS_CSV):
        for r in load_csv(CANONICAL_ALL_EVENTS_CSV):
            eid = r.get("event_id", "").strip()
    ...
    cols   = ["Event ID", "Year", "Event Name", "Location", "Reason", "Status", "Source"]
    widths = [32, 6, 52, 28, 20, 14, 10]
    for c, (h, w) in enumerate(zip(cols, widths), start=1):
        cell = ws.cell(row=1, column=c, value=h)
        cell.font      = _IDX_FONT_HDR
        cell.fill      = _IDX_FILL_HDR
    ...
    row_idx = 2
    _FILL_EXCL_NO_RES = PatternFill("solid", fgColor="FFF9C4")   # pale yellow
    _FILL_EXCL_SPARSE = PatternFill("solid", fgColor="F5F5F5")   # light gray

    excluded = []
    for eid, r in all_events.items():
        if eid in published_ids and eid not in quarantine_set:
            continue
        status = r.get("status", "")
        if status == "no_results":
            reason = "no_results"
        elif eid in quarantine_set:
            reason = "quarantined"
        else:
            reason = "all_sparse"
        ...
        if city and country in ("United States", "Canada") and region:
            ...
            loc = r.get("location", "").strip() or country

        excluded.append({
            "eid": eid, "year": r.get("year", ""), "name": r.get("event_name", ""),
            "loc": loc, "reason": reason, "status": status,
            "source": r.get("data_source", ""),
        })

    excluded.sort(key=lambda x: (x["year"], x["name"].lower()))
    for ex in excluded:
        fill = _FILL_EXCL_NO_RES if ex["reason"] == "no_results" else _FILL_EXCL_SPARSE
        vals = [ex["eid"], ex["year"], ex["name"], ex["loc"],
                ex["reason"], ex["status"], ex["source"]]
    ...
    n = row_idx - 2
    print(f"  EXCLUDED EVENTS: {n} rows")
    return n


# ── RECORDS sheets ────────────────────────────────────────────────────────────

_REC_FILL_HDR   = PatternFill("solid", fgColor="1F3864")
_REC_FONT_HDR   = Font(bold=True, size=10, color="FFFFFF")
_REC_FONT_NORM  = Font(size=10)
_REC_FONT_NOTE  = Font(italic=True, size=9, color="666666")
_REC_FONT_TITLE = Font(bold=True, size=13)
_REC_ALIGN_L    = Alignment(horizontal="left", vertical="top")
_REC_ALIGN_C    = Alignment(horizontal="center", vertical="top")


def _rec_hrow(ws, row: int, headers: list, widths: list) -> int:
    for c, (h, w) in enumerate(zip(headers, widths), start=1):
        cell = ws.cell(row=row, column=c, value=h)
        cell.font      = _REC_FONT_HDR
        cell.fill      = _REC_FILL_HDR
        cell.alignment = _REC_ALIGN_C
        ws.column_dimensions[get_column_letter(c)].width = w
    return row + 1


def build_records_sheets(wb: Workbook) -> None:
    """Add RECORDS SUMMARY, RECORDS LEADERBOARD, RECORDS REVIEW QUEUE, and
    RECORDS UNRESOLVED PLAYERS sheets from early_data/out/records/ CSVs.
    Fails gracefully if any CSV is absent."""

    if not os.path.exists(RECORDS_SUMMARY_CSV):
        print("  [WARN] Records CSVs not found — skipping records sheets")
        return

    # Position all records sheets before the first year sheet
    def _insert_before_years(wb: Workbook, name: str):
        if name in wb.sheetnames:
            del wb[name]
        first_yr = next((s for s in wb.sheetnames if s.isdigit()), None)
        ins_idx = wb.sheetnames.index(first_yr) if first_yr else len(wb.sheetnames)
        return wb.create_sheet(name, ins_idx)

    # ── RECORDS SUMMARY ──────────────────────────────────────────────────────
    sheet_name = "RECORDS SUMMARY"
    ws = _insert_before_years(wb, sheet_name)

    rows = load_csv(RECORDS_SUMMARY_CSV)
    ws.cell(row=1, column=1, value="Consecutives World Records — Summary by Trick").font = _REC_FONT_TITLE
    ws.merge_cells("A1:K1")
    ws.cell(row=2, column=1, value=(
        "Each row is one trick. Top record value and current record holder shown. "
        "Multiple holders listed when tied."
    )).font = _REC_FONT_NOTE
    ws.merge_cells("A2:K2")

    hdrs   = ["Trick", "Sort Name", "Adds", "Top Value", "Record ID", "Top Holder",
              "# Records", "# Holders", "First Date", "Latest Date", "All Holders"]
    widths = [28, 28, 6, 10, 36, 26, 10, 10, 13, 13, 60]
    fields = ["trick_name", "sort_name", "adds", "top_record_value", "top_record_id",
              "top_holder", "record_count", "holders_count", "first_date", "latest_date", "holders"]
    data_row = _rec_hrow(ws, 3, hdrs, widths)
    for r in rows:
        for c, f in enumerate(fields, start=1):
            ...
            cell.font      = _REC_FONT_NORM
            cell.alignment = _REC_ALIGN_L
        data_row += 1
    ws.freeze_panes = "A4"
    ws.auto_filter.ref = f"A3:{get_column_letter(len(hdrs))}3"
    print(f"  RECORDS SUMMARY: {len(rows)} tricks")

    # ── RECORDS LEADERBOARD ───────────────────────────────────────────────────
    sheet_name = "RECORDS LEADERBOARD"
    ws = _insert_before_years(wb, sheet_name)

    rows = load_csv(RECORDS_LEADER_CSV)
    ws.cell(row=1, column=1, value="Consecutives World Records — Leaderboard by Trick").font = _REC_FONT_TITLE
    ws.merge_cells("A1:K1")
    ws.cell(row=2, column=1, value=(
        "One row per record holder per trick, ranked within each trick. "
        "Ties share the same rank."
    )).font = _REC_FONT_NOTE
    ws.merge_cells("A2:K2")

    hdrs   = ["Trick", "Sort Name", "Adds", "Rank", "Player", "Player ID",
              "Value", "Date", "Video", "Time Clip", "Record ID"]
    widths = [28, 28, 6, 6, 26, 38, 8, 13, 48, 10, 36]
    fields = ["trick_name", "sort_name", "adds", "rank", "player", "player_id",
              "record_value", "date_normalized", "video", "time_clip", "record_id"]
    data_row = _rec_hrow(ws, 3, hdrs, widths)
    for r in rows:
        for c, f in enumerate(fields, start=1):
            v = r.get(f, "")
    ...
    ws.freeze_panes = "A4"
    ws.auto_filter.ref = f"A3:{get_column_letter(len(hdrs))}3"
    print(f"  RECORDS LEADERBOARD: {len(rows)} rows")

    # ── RECORDS REVIEW QUEUE ──────────────────────────────────────────────────
    sheet_name = "RECORDS REVIEW QUEUE"
    ws = _insert_before_years(wb, sheet_name)

    rows = load_csv(RECORDS_QUEUE_CSV)
    ws.cell(row=1, column=1, value="Consecutives Records — Review Queue").font = _REC_FONT_TITLE
    ws.merge_cells("A1:W1")
    ws.cell(row=2, column=1,
            value="Records flagged for manual review. Check reason column for issue details."
           ).font = _REC_FONT_NOTE
    ws.merge_cells("A2:W2")

    hdrs   = ["Record ID", "Discipline", "Type", "Category", "Subcategory", "Trick",
              "Sort Name", "Value", "Unit", "Adds", "Player", "Player ID", "Date",
              "Approx?", "Event ID", "Video", "Time Clip", "Source", "Source URL",
              "Verification", "Confidence", "Notes", "Review Reason"]
    widths = [36, 12, 16, 26, 20, 26, 26, 8, 8, 6, 24, 38, 13,
              8, 32, 48, 10, 12, 48, 14, 12, 36, 36]
    fields = ["record_id", "discipline", "record_type", "category", "subcategory", "trick_name",
              "sort_name", "record_value", "unit", "adds", "player", "player_id", "date_normalized",
              "approx_date", "event_id", "video", "time_clip", "source", "source_url",
              "verification_status", "confidence", "notes", "review_reason"]
    data_row = _rec_hrow(ws, 3, hdrs, widths)
    _FILL_QUEUE = PatternFill("solid", fgColor="FFF3E0")
    for r in rows:
        for c, f in enumerate(fields, start=1):
            v = r.get(f, "")
            cell = ws.cell(row=data_row, column=c, value=v)
            cell.font      = _REC_FONT_NORM
            cell.alignment = _REC_ALIGN_L
            cell.fill      = _FILL_QUEUE
        data_row += 1
    ws.freeze_panes = "A4"
    if rows:
        ws.auto_filter.ref = f"A3:{get_column_letter(len(hdrs))}3"
    print(f"  RECORDS REVIEW QUEUE: {len(rows)} rows")

    # ── RECORDS UNRESOLVED PLAYERS ────────────────────────────────────────────
    sheet_name = "RECORDS UNRESOLVED PLAYERS"
    rows = load_csv(RECORDS_UNRES_CSV)
    if rows:
        ws = _insert_before_years(wb, sheet_name)
        ws.cell(row=1, column=1,
                value="Records — Players Not Matched to Canonical Identity").font = _REC_FONT_TITLE
        ws.merge_cells("A1:C1")

        hdrs   = ["Player (raw)", "Normalised", "Records Count"]
        widths = [36, 36, 14]
        data_row = _rec_hrow(ws, 2, hdrs, widths)
        for r in rows:
            for c, f in enumerate(["player", "player_norm", "records_count"], start=1):
                v = r.get(f, "")
                try:
                    v = int(v) if str(v).isdigit() else v
                except (AttributeError, ValueError):
            ...
            data_row += 1
        ws.freeze_panes = "A3"
        print(f"  RECORDS UNRESOLVED PLAYERS: {len(rows)} rows")
    else:
        print(f"  RECORDS UNRESOLVED PLAYERS: empty — skipped")


# ── CORRECTIONS sheet ─────────────────────────────────────────────────────────

def build_corrections_sheet(wb: Workbook) -> None:
    """
    Document verified corrections to historical records.
    Currently covers: 1987 Worlds fix, 1995/1996 Worlds corrections.
    """
    sheet_name = "CORRECTIONS"
    if sheet_name in wb.sheetnames:
        del wb[sheet_name]
    # Position before first year sheet
    first_yr = next((s for s in wb.sheetnames if s.isdigit()), None)
    ins_idx = wb.sheetnames.index(first_yr) if first_yr else len(wb.sheetnames)
    ws = wb.create_sheet(sheet_name, ins_idx)

    FONT_T  = Font(bold=True, size=13)
    FONT_S  = Font(bold=True, size=11)
    FONT_B  = Font(size=11)
    FONT_N  = Font(italic=True, size=10, color="666666")
    FILL_S  = _fill("D9E1F2")
    FILL_R  = _fill("FFF3E0")
    ALIGN_W = Alignment(wrap_text=True, vertical="top")
    ALIGN_L = Alignment(vertical="top")
    ...
    def kv(label, value):
        a = ws.cell(row=row[0], column=1, value=label)
        a.font = FONT_S
        a.alignment = ALIGN_L
        b = ws.cell(row=row[0], column=2, value=value)
        b.font = FONT_B
        b.alignment = ALIGN_W
        b.fill = FILL_R
        ws.row_dimensions[row[0]].height = max(15, min(60, len(str(value)) // 3))
        row[0] += 1
    ...
    title("Verified Historical Corrections")
    note("This sheet documents cases where the source data was found to be incorrect "
         "and has been replaced with verified information.")

    section("1987 World Footbag Championships (Correction applied)")
    kv("Issue",
       "The Footbag.org archive contained two entries for '1988 World Footbag Championships'. "
       "After cross-referencing with Footbag World magazine, one entry was confirmed to be the "
       "actual 1987 Worlds (mislabeled in the digital archive).")
    kv("Action",
       "The mislabeled 1988 entry has been removed. The corrected 1987 entry is included with "
       "verified results from Footbag World magazine, including Sébastien Verdy as the winner "
       "of Open Freestyle.")
    kv("Source", "Footbag World magazine scans, cross-referenced with event chronology.")

    section("1995 IFAB World Footbag Championships (Removed)")
    kv("Issue",
       "A 1995 IFAB Worlds entry appeared in the archive. IFAB (International Footbag "
       "Association of Boardgames — a lesser-known parallel sanctioning body) ran a "
       "competing championship. After review, this event was excluded to avoid confusion "
       "with the mainstream IFA (International Footbag Association) Worlds held the same year.")
    kv("Action",
       "The 1995 IFAB Worlds entry has been removed from the dataset. The 1995 IFA World "
       "Footbag Championships remains and is the canonical 1995 Worlds event.")
    kv("Source", "Footbag World magazine and community expert review.")

    section("1996 World Footbag Championships (Results corrected)")
    kv("Issue",
       "The Footbag.org archive entry for the 1996 Worlds had incomplete or incorrect "
       "results. Footbag World magazine contained verified final standings.")
    kv("Action",
       "Results replaced with verified data from Footbag World magazine. "
       "Sébastien Verdy confirmed as Open Freestyle winner.")
    kv("Source", "Footbag World magazine, issue covering the 1996 World Championships.")

    ws.freeze_panes = "A2"
    print("  CORRECTIONS sheet written")
...
def main():
    ...
    # Count only events that have qualifying data (>= 3 placements per discipline)
    _pf_event_ids    = {r.get("event_id", "") for r in pf_rows}
    _event_count     = len(_pf_event_ids)
    ...
    # Count excluded events (in canonical_all but not in release_publication)
    _all_event_count = 0
    if os.path.exists(CANONICAL_ALL_EVENTS_CSV):
        with open(CANONICAL_ALL_EVENTS_CSV, encoding="utf-8") as _f:
            _all_event_count = sum(1 for _ in csv.DictReader(_f))
    _excluded_count = max(0, _all_event_count - _event_count)
    ...
    build_data_notes(out_wb, fffd_count=_fffd_count, quarantine_count=len(yr_quarantine),
                     published_count=_event_count, excluded_count=_excluded_count)
    ...
    # Build supplemental transparency sheets
    print("\nBuilding EXCLUDED EVENTS sheet...")
    # Note: yr_events was mutated by build_event_index to include excluded events;
    # use _pf_event_ids (the originally published set) as the baseline.
    build_excluded_events_sheet(out_wb, _pf_event_ids, placements_by_event, yr_quarantine)

    print("\nBuilding CORRECTIONS sheet...")
    build_corrections_sheet(out_wb)

    print("\nBuilding RECORDS sheets...")
    build_records_sheets(out_wb)
