...
• Data sources updated:
  - PF_CSV       → out/canonical_pf.csv          (was merged_placements_flat.csv)
  - CANON_EVENTS → out/release_publication/events.csv  (was out/canonical/events.csv)
  - OUT_HTML     → out/event_comparison_viewer_v12.html
• PF join key: canonical_pf.csv uses slug IDs as event_id; build_events() now joins
  on slug (event_key from stage2) instead of numeric ID.
• JPG/image viewer removed entirely: SOURCE_MAP_FILE, GEMINI_TRUSTED_CSV,
  GEMINI_IMG_DIR, load_source_map(), load_gemini_trusted(), _EMPTY_PROV,
  provenance strip, scan pane, canvas/bbox/rotation JS all removed.
• Layout is now 2-column (mirror text left, canonical right).
• Source-type and confidence-tier filter dropdowns removed from controls bar.
• Pre-1997 CANON_EVENTS loop updated: uses event_id (slug) as primary key field.

V11 additions over V10
-----------------------
• Source provenance: loads out/pdf_compare/event_to_source_map.json and attaches
  source_type, confidence_tier, evidence_summary, scan_jpg, source_page to each event.
• Provenance strip rendered below event key (compact 1-2 line area).
• List items show a small source-type badge (colored by tier/status).
• Header bar: two new filter dropdowns — Source Type and Confidence Tier.
• Unresolved events (no mapped source) show "source: mirror_only / unresolved".
• Scan image pane (3rd column, 440px): shows JPEG when src_jpg_rel is available; empty otherwise.
  - Canvas-based rendering: image drawn onto <canvas> so rotation and bbox overlay compose correctly.
  - Rotation controls (↺ / ↻) above canvas; rotation stored as 0/90/180/270 degrees.
  - BBox overlay: if src_bbox=[x1,y1,x2,y2] present, draws red rect with transparent fill scaled to
    displayed image; corners transformed through rotation matrix so overlay survives rotation.
  - Auto-focus: if bbox present, pane scrolls so bbox center is vertically centered on load.
• QC alignment, comparison layout, and row classification logic unchanged from V10.
...
PF_CSV          = OUT / "canonical_pf.csv"
QUARANTINE_CSV  = ROOT / "inputs" / "review_quarantine_events.csv"
CANON_EVENTS    = OUT / "release_publication" / "events.csv"
OUT_HTML        = OUT / "event_comparison_viewer_v12.html"
...
def load_pf():
    """Return PF rows indexed by event_id (slug) — same key as stage2 event_key."""
...
def build_events(quarantine, pf):
    events = []
    seen_slugs = set()
    ...
    with open(STAGE2_CSV, newline='', encoding='utf-8') as f:
        for r in csv.DictReader(f):
            ...
            pf_rows         = pf.get(event_id, [])   # join on slug ID
            ...
            seen_slugs.add(event_id)

    # Also include pre-1997 canonical events absent from merged_stage2.csv.
    # These have no mirror results to compare, but are in the canonical dataset.
    if CANON_EVENTS.exists():
        with open(CANON_EVENTS, newline='', encoding='utf-8') as f:
            for r in csv.DictReader(f):
                slug = r.get('event_id', '') or r.get('event_key', '')
                slug = slug.strip()
...
def main():
    ...
    events = build_events(quarantine, pf)
