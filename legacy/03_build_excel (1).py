...
# Excel is also touchy about these in some contexts:
_EXTRA_BAD = {
    "\r": "\n",      # normalize CR -> LF
    "\t": " ",       # tabs -> spaces
    "\u2028": "\n",  # line separator
...
# XML-unsafe in cell text can trigger "unreadable content" even if openpyxl usually escapes
_XML_UNSAFE = [("&", " and "), ("<", " "), (">", " "), ('"', "'"), ("\x7f", "")]

try:
    from openpyxl.cell.cell import ILLEGAL_CHARACTERS_RE as _OPENPYXL_ILLEGAL
except ImportError:
    _OPENPYXL_ILLEGAL = None


def sanitize_string(s: str) -> str:
    """Sanitize a single string for Excel (file-safety only, not semantic cleaning)."""
    if not isinstance(s, str):
        return s

    # Remove forbidden control chars (ours + openpyxl's if available)
    s = _ILLEGAL_XLSX_RE.sub("", s)
    if _OPENPYXL_ILLEGAL is not None:
        s = _OPENPYXL_ILLEGAL.sub("", s)

    # Normalize other known Excel-troublemakers
    for bad, repl in _EXTRA_BAD.items():
        if bad in s:
            s = s.replace(bad, repl)

    # Replace XML-unsafe chars so stored value cannot break XML
    for bad, repl in _XML_UNSAFE:
        if bad in s:
            s = s.replace(bad, repl)

    # Collapse crazy whitespace runs (keeps newlines)
    lines = [re.sub(r"[ \f\v]+", " ", ln).rstrip() for ln in s.split("\n")]
    s = "\n".join(lines).strip()

    # Prevent Excel from interpreting as formula (causes "Removed Records: Formula" repair)
...
def _sanitize_results_cell(s: str) -> str:
    """Gentler sanitization for Results cell: keep Unicode names (e.g. Tantarimäki, Šimkù).
    Skip openpyxl ILLEGAL (can strip valid accented chars); keep control-char and XML safety."""
    if not isinstance(s, str):
        return s
    s = _ILLEGAL_XLSX_RE.sub("", s)
    for bad, repl in _EXTRA_BAD.items():
...
def sanitize_excel_strings(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Results row uses gentler _sanitize_results_cell to preserve Unicode in names."""
    if df.empty:
        return df
    out = df.copy()
    has_results_row = "Results" in out.index
        ...
        if pd.api.types.is_string_dtype(out[col]) or out[col].dtype == object:
            if has_results_row:
                for idx in out.index:
                    v = out.loc[idx, col]
                    if isinstance(v, str):
                        out.loc[idx, col] = _sanitize_results_cell(v) if idx == "Results" else sanitize_string(v)
            else:
                out[col] = out[col].apply(
                    lambda v: sanitize_string(v) if isinstance(v, str) else v
                )
    return out
...
def year_to_sheet_name(y) -> str:
    ...
    try:
        ...
        return _sanitize_sheet_name(str(yi))
    except Exception:
        return _sanitize_sheet_name(str(y))
...
# Excel cell character limit (hard limit; exceeding causes truncation or errors)
EXCEL_CELL_CHAR_LIMIT = 32767

def _cap_results_for_excel_cell(text: Optional[str]) -> str:
    """Ensure results string fits in one Excel cell; add note if truncated."""
    if not text or not isinstance(text, str):
        return ""
    if len(text) <= EXCEL_CELL_CHAR_LIMIT:
        return text
    suffix = "\n[... truncated; exceeds Excel cell limit (32,767 characters) ...]"
    return text[: EXCEL_CELL_CHAR_LIMIT - len(suffix)] + suffix
...
def _flat_rows_to_placements(pf_subset: pd.DataFrame) -> list[dict]:
    """
    Convert a DataFrame of Placements_Flat rows (one event) into the list-of-dict
    format expected by format_results_from_placements. Uses person_canon and
    team_display_name for display (identity-lock canonical names).
    """
    if pf_subset is None or pf_subset.empty:
        return []
    out = []
    for _, row in pf_subset.iterrows():
        def _str(v):
            if v is None or (isinstance(v, float) and pd.isna(v)):
                return ""
            return str(v).strip()

        div_canon = _str(row.get("division_canon")) or "Unknown"
        div_cat = _str(row.get("division_category")) or "unknown"
        place = row.get("place")
        person_canon = _str(row.get("person_canon"))
        team_display = _str(row.get("team_display_name"))
        out.append({
...
def _build_name_line(placement: dict, players_by_id: Optional[dict] = None) -> str:
    ...
    Supports both stage2 placement dicts (player1_name, player2_name) and
    Placements_Flat-style dicts (person_canon, team_display_name)."""

    # Identity-lock / Placements_Flat schema: prefer canonical display names
    team_display = (placement.get("team_display_name") or "").strip()
    if team_display:
        return team_display
    person_canon = (placement.get("person_canon") or "").strip()
    if person_canon:
        return person_canon
...
def write_excel(
    ...
    # Normalize event_id so we match even when CSV/DataFrame has int/float (e.g. 1449259560.0)
    def _norm_eid(v) -> str:
        if v is None or (isinstance(v, float) and pd.isna(v)):
            return ""
        try:
    ...
    # Build results map: prefer Placements_Flat (identity-lock) when available so the
    # workbook shows full canonical results (e.g. worlds with net + freestyle); else stage2.
    # Use normalized event_id as key so lookup works whether sheet uses "1449259560" or "1449259560.0".
    results_map = {}
    from_flat_count = 0
    from_stage2_count = 0
    flat_event_sample = []  # (eid_canon, n_flat_rows, result_len) for first few
    stage2_only_eids = []   # event_ids with no rows in Placements_Flat
    for rec in records:
        eid = rec.get("event_id")
        if not eid:
            continue
        eid_canon = _norm_eid(eid)
        if not eid_canon:
            continue
        if placements_flat_df is not None and not placements_flat_df.empty:
            # Match on normalized event_id (handles "1449259560" and "1449259560.0")
            pf_event_ids_norm = placements_flat_df["event_id"].apply(lambda v: _norm_eid(v) if pd.notna(v) else "")
            pf_for_event = placements_flat_df[pf_event_ids_norm == eid_canon]
            if not pf_for_event.empty:
                n_flat = len(pf_for_event)
                flat_placements = _flat_rows_to_placements(pf_for_event)
                text = format_results_from_placements(flat_placements, players_by_id)
                results_map[eid_canon] = text
                from_flat_count += 1
                if len(flat_event_sample) < 5:
                    flat_event_sample.append((eid_canon, n_flat, len(text) if text else 0))
                # Diagnostic for events with many Flat rows (e.g. worlds 1449259560)
                if eid_canon == "1449259560" or n_flat >= 25:
                    n_lines = (text or "").count("\n") + (1 if (text or "").strip() else 0)
                    print(f"    [diag] event {eid_canon}: {n_flat} Flat rows -> {len(text or '')} chars, ~{n_lines} lines")
                continue
        placements = rec.get("placements", [])
        results_map[eid_canon] = format_results_from_placements(placements, players_by_id)
        from_stage2_count += 1
        stage2_only_eids.append(eid_canon)

    print(f"  Results: {from_flat_count} events from Placements_Flat, {from_stage2_count} from stage2")
    for eid_c, n_flat, n_chars in flat_event_sample:
        print(f"    Placements_Flat used for event {eid_c}: {n_flat} rows -> {n_chars} chars")
    if stage2_only_eids:
        print(f"  Events with no Placements_Flat rows (using stage2 only): {stage2_only_eids[:15]}{'...' if len(stage2_only_eids) > 15 else ''}")
    # Where does event 1449259560 end up? (so we know which sheet to check)
    for rec in records:
        if _norm_eid(rec.get("event_id")) == "1449259560":
            y = rec.get("year")
            print(f"  [diag] event 1449259560 in records -> sheet: {year_to_sheet_name(y) if y is not None else 'unknown_year'}")
            break
    ...
    # Sort key for event IDs (tiebreaker when dates match or are missing)
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as xw:
        ...
        for y in sorted(by_year.keys()):
            ...
            for eid in eids:
                ...
                col_key = int(eid) if eid.isdigit() else sanitize_string(eid)
                results_text = results_map.get(_norm_eid(eid)) or results_map.get(eid) or ""
                results_text = _cap_results_for_excel_cell(results_text)
                if _norm_eid(eid) == "1449259560":
                    print(f"    [diag] Writing event 1449259560: results len={len(results_text)} (sanitize next)")
                results_sanitized = _sanitize_results_cell(results_text)
                if _norm_eid(eid) == "1449259560" and len(results_sanitized) != len(results_text):
                    print(f"    [diag] event 1449259560: sanitize changed length {len(results_text)} -> {len(results_sanitized)}")
                data[col_key] = [
            ...
            # Column order must match eids so each event's results go to the correct column
            col_order = [int(e) if e.isdigit() else sanitize_string(e) for e in eids]
            df_year = pd.DataFrame(data, index=row_labels, columns=col_order)
            df_year.index.name = "event_id"  # puts "event_id" in first column header

            df_year = sanitize_excel_strings(df_year)
            # Write with startrow=1 so row 1 is free for banner; avoids insert_rows() which can break sheet dimension
            df_year.to_excel(xw, sheet_name=sheet_name, startrow=1)

            worksheet = xw.sheets[sheet_name]
            n_cols = len(eids) + 1  # col A (labels) + one per event
            last_col = get_column_letter(n_cols)
            # Row 1 = year banner (no insert_rows, so dimension stays correct)
            worksheet.merge_cells(f"A1:{last_col}1")
            banner = worksheet["A1"]
            banner.value = int(y)
            banner.alignment = Alignment(horizontal="center", vertical="center")

            # Write Results row directly with openpyxl so long text is not truncated (pandas/to_excel can truncate)
            results_row = 8  # startrow=1 + header row + 5 data rows (Tournament Name, Date, Location, Event Type, Host Club)
            for j, eid in enumerate(eids):
                results_text = results_map.get(_norm_eid(eid)) or results_map.get(eid) or ""
                results_text = _cap_results_for_excel_cell(results_text)
                results_text = _sanitize_results_cell(results_text)
                cell = worksheet.cell(row=results_row, column=2 + j)
                cell.value = results_text
                a = copy(cell.alignment)
                a.wrap_text = True
                cell.alignment = a
            worksheet.row_dimensions[results_row].height = 409  # Excel max; more Results lines visible
        ...
        if unknown_year:
            ...
            for eid in eids:
                ...
                data[col_key] = [
                    ...
                    sanitize_string(rec.get("event_type") or ""),
                    sanitize_string(rec.get("host_club") or ""),
                    _sanitize_results_cell(results_text),
                ]
                if _norm_eid(eid) == "1449259560":
                    print(f"    [diag] Writing event 1449259560 (unknown_year sheet): results len={len(results_text)}")

            col_order = [int(e) if e.isdigit() else sanitize_string(e) for e in eids]
            df_unk = pd.DataFrame(data, index=row_labels, columns=col_order)
            df_unk.index.name = "event_id"
            df_unk = sanitize_excel_strings(df_unk)
            df_unk.to_excel(xw, sheet_name="unknown_year")

            # Write Results row directly with openpyxl so long text is not truncated
            worksheet = xw.sheets["unknown_year"]
            results_row_unk = 7  # no banner; header row 1 + 5 data rows
            for j, eid in enumerate(eids):
                results_text = results_map.get(_norm_eid(eid)) or results_map.get(eid) or ""
                results_text = _cap_results_for_excel_cell(results_text)
                results_text = _sanitize_results_cell(results_text)
                cell = worksheet.cell(row=results_row_unk, column=2 + j)
                cell.value = results_text
                a = copy(cell.alignment)
                a.wrap_text = True
                cell.alignment = a
            worksheet.row_dimensions[results_row_unk].height = 409  # Excel max; more Results lines visible

        # Build Index sheet: one row per event with hyperlinks
        index_data = []
        sce_event_ids = {_norm_eid(rec.get("event_id")) for rec in records if rec.get("event_id")}
        for rec in records:
            eid = str(rec.get("event_id", ""))
            if not eid:
                continue
        ...
        if placements_flat_df is not None and not placements_flat_df.empty:
            for _eid, _grp in placements_flat_df.groupby(placements_flat_df["event_id"].apply(_norm_eid)):
                if _eid:
                    pf_count_by_event[_eid] = len(_grp)

        # Back-fill placements_count in the already-built index_data rows using PBP counts.
        for row in index_data:
            row["placements_count"] = pf_count_by_event.get(_norm_eid(row.get("event_id")), 0)
        ...
        if placements_flat_df is not None and not placements_flat_df.empty:
            pf_event_ids = placements_flat_df["event_id"].dropna().apply(_norm_eid).unique()
            pf_event_ids = [x for x in pf_event_ids if x]
            for eid in sorted(pf_event_ids, key=_eid_sort_key):
                if eid in sce_event_ids:
                    continue
                pf_rows = placements_flat_df[placements_flat_df["event_id"].apply(_norm_eid) == eid]
        ...
        # Use Hyperlink(ref=..., location=...) so links are stored as relationships, not formulas
        # (formulas in sheet42 were being removed by Excel repair).
        ...
        for idx, row_data in enumerate(index_data, start=2):
            ...
            if eid in event_locator:
                ...
                safe_sheet = sheet_name.replace("'", "''")  # escape single quotes in sheet name
                cell_ref = f"A{idx}"
                location = f"'{safe_sheet}'!{col_letter}1"
                cell = index_ws.cell(row=idx, column=1)  # Column A
                cell.hyperlink = Hyperlink(ref=cell_ref, location=location)
        ...
        # Auto-filter omitted to avoid Excel repair dialog (structural XML issues)
        ...
        # summary_ws.auto_filter.ref = None  # no filter on Summary
        ...
        if players_df is not None and len(players_df) > 0:
            ...
            if candidates_rows:
                ...
                ws.freeze_panes = "A2"
                # ws.auto_filter.ref = ws.dimensions  # omitted: triggers Excel repair
        ...
        # divisions_ws.auto_filter.ref = divisions_ws.dimensions  # omitted: triggers Excel repair
        ...
        if teams_data:
            ...
            # teams_ws.auto_filter.ref = teams_ws.dimensions  # omitted: triggers Excel repair
        ...
        if qc_summary_path.exists() and qc_issues_path.exists():
            try:
                ...
                if qc_ws:
                    ...
                    # qc_ws.auto_filter.ref = qc_ws.dimensions  # omitted: triggers Excel repair
        ...
        # ws.auto_filter.ref = ws.dimensions  # omitted: triggers Excel repair (README sheet)
...
def _diagnose_xlsx(path: Path) -> None:
    """
    Scan the saved workbook for cell values that often trigger Excel's repair dialog.
    Reports the first offending cell so you can fix the source or sanitization.
    """
    if not path.exists():
        return
    try:
        ...
        print(f"Diagnostic: could not open workbook: {e}")
        return
    try:
        control_re = re.compile(r"[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]")
        xml_unsafe = re.compile(r"[&<>]")
            ...
            for row in ws.iter_rows():
                for cell in row:
                    ...
                    if not isinstance(val, str):
                        continue
                    if control_re.search(val):
                        addr = f"{sheet_name}!{cell.coordinate}"
                        preview = repr(val[:60]) + ("..." if len(val) > 60 else "")
                        print(f"Diagnostic: first problematic cell {addr} (control char): {preview}")
                        return
                    if xml_unsafe.search(val):
                        addr = f"{sheet_name}!{cell.coordinate}"
                        preview = repr(val[:60]) + ("..." if len(val) > 60 else "")
                        print(f"Diagnostic: first problematic cell {addr} (XML-unsafe & < >): {preview}")
                        return
        print("Diagnostic: no control or XML-unsafe characters found in cell values.")
    finally:
        wb.close()
...
def main():
    ...
    # Keep event_id as string so matching with records is reliable (no 1449259560 vs 1449259560.0)
    df_placements_flat = pd.read_csv(placements_flat_csv, dtype={"event_id": str})
    df_placements_flat["event_id"] = df_placements_flat["event_id"].fillna("").astype(str).str.strip()
    print(f"Loaded {placements_flat_csv} ({len(df_placements_flat)} rows)")

    print(f"Writing Excel with {len(records)} events...")
    results_map = write_excel(
        out_xlsx, records, players_df=players_df,
        placements_flat_df=df_placements_flat, unresolved_df=df_unresolved
    )

    # Run Stage 3 QC on workbook data (use same results_map that was written; check for dropped data)
    if USE_MASTER_QC and results_map is not None:
        qc_summary, qc_issues = qc_master.run_qc_for_stage(
            "stage3", records, results_map=results_map, players_by_id=players_by_id,
            placements_flat_df=df_placements_flat, out_dir=OUT_DIR
        )
    ...
    # Read back Results cell length for event 1449259560 (before post-save)
    try:
        wb = load_workbook(out_xlsx, read_only=False, data_only=True)
        ws = wb["2016"]
        # Header row is 2; Results row is 8 (startrow=1 + header + 5 data rows)
        for col in range(2, ws.max_column + 1):
            header_val = ws.cell(row=2, column=col).value
            if header_val is not None and str(header_val).strip() in ("1449259560", "1449259560.0"):
                val = ws.cell(row=8, column=col).value
                ln = len(str(val)) if val is not None else 0
                print(f"  [readback] Before post-save: event 1449259560 Results cell len={ln}")
                break
        wb.close()
    except Exception as e:
        print(f"  [readback] Could not check cell: {e}")

    # Post-save normalization so Excel opens without repair dialog (can truncate cells; skip to test)
    try:
        wb = load_workbook(out_xlsx)
        wb.save(out_xlsx)
        print("Post-save normalization complete (Excel repair dialog should be gone).")
    except Exception as e:
        print(f"Warning: post-save normalization failed: {e}")

    # Read back again after post-save
    try:
        wb = load_workbook(out_xlsx, read_only=False, data_only=True)
        ws = wb["2016"]
        for col in range(2, ws.max_column + 1):
            ...
            if header_val is not None and str(header_val).strip() in ("1449259560", "1449259560.0"):
                val = ws.cell(row=8, column=col).value
                ln = len(str(val)) if val is not None else 0
                print(f"  [readback] After post-save:  event 1449259560 Results cell len={ln}")
                break
        wb.close()
    except Exception as e:
        print(f"  [readback] Could not check after save: {e}")

    # Diagnostic: scan for characters that often trigger Excel repair dialog
    _diagnose_xlsx(out_xlsx)
