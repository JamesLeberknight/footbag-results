...
INPUTS (required):
  - out/stage1_raw_events.csv   Stage 1 raw events (run 01_parse_mirror.py or 01c_merge_stage1.py first)

INPUTS (optional):
  - inputs/location_canon_full_final.csv   Event ID -> canonical location (city_canon, state_canon, country_canon)
  - overrides/events_overrides.jsonl       Per-event overrides (year, event_name, date, location, host_club, event_type, exclude)
  - data/qc_baseline_stage2.json           QC baseline for regression check (created with --save-baseline)

OUTPUTS:
  - out/stage2_canonical_events.csv    Canonical events with placements_json
  - out/stage2_players.csv             Player registry (player_id, player_name, country_observed)
  - out/stage2_qc_summary.json          QC summary (errors/warnings counts, field coverage)
  - out/stage2_qc_issues.jsonl          QC issues (one JSON object per line)
  - data/qc_baseline_stage2.json        Only when --save-baseline

REQUIREMENTS:
  - Python 3, run from repo root (or scripts/ with PYTHONPATH set)
  - qc/qc_master.py importable for full QC; falls back to embedded QC if missing
"""
...
def categorize_division(division_name: str, event_type: str = None) -> str:
    ...
    # Net-structural division names (Mixed Doubles, Open Singles, etc.) are never golf.
    # Do not let event_type=golf override these when event name happens to mention "golf".
    if "mixed" in low and ("double" in low or "single" in low):
        return "net"
    if any(phrase in low for phrase in ("open single", "open double", "doubles net", "singles net")):
        return "net"
    # "Singles" / "Doubles" / "Mixed" as standalone structural names (worlds-style) → net
    if re.search(r"\b(singles?|doubles?|mixed)\b", low) and "golf" not in low and "golfer" not in low:
        # Only if we would otherwise be guessing from event_type - treat as net by convention
        if event_type and event_type.lower() == "golf":
            return "net"  # Golf has no "Mixed Doubles"; this is net
...
def parse_results_text(results_text: str, event_id: str, event_type: str = None) -> list[dict]:
    ...
    for raw_line in (results_text or "").splitlines():
        ...
        # Skip lines that look like years (e.g., "2007 US Open..." parsed as place=2007)
        # Allow places 100+ in large divisions (e.g. Worlds); only drop year-like values
        if 1990 <= place <= 2030:
            continue  # Likely a year misparsed as place
...
def infer_event_type(event_name: str, results_raw: str, placements: list = None) -> str:
    ...
    Priority:
    1. "World Footbag Championships" in name → "worlds" only if placements
       contain both net and freestyle (worlds must have both; otherwise "mixed").
    ...
    # If we have placements, use their division categories first (needed for worlds check)
    if placements:
        categories = set()
        for p in placements:
        ...
        # Worlds: only if name says so AND we have both net and freestyle
        if "world footbag championship" in name_lower:
            if has_net and has_freestyle:
                return "worlds"
            return "mixed"  # Name says worlds but data incomplete → don't claim worlds
...
def write_qc_outputs(summary: dict, issues: list[dict], out_dir: Path) -> None:
    ...
    _write_qc_outputs_quiet(summary, issues, out_dir)
    print(f"Wrote: {out_dir / 'stage2_qc_summary.json'}, {out_dir / 'stage2_qc_issues.jsonl'} ({len(issues)} issues)")


def _write_qc_outputs_quiet(summary: dict, issues: list[dict], out_dir: Path) -> None:
    """Write QC summary and issues to output files (no console output)."""
...
def _qc_regressions(current: dict, baseline: dict) -> list[str]:
    """Return check_ids where ERROR count increased vs baseline (no printing)."""
    regressions = []
    baseline_checks = baseline.get("counts_by_check", {})
    current_checks = current.get("counts_by_check", {})
    for check_id in set(baseline_checks.keys()) | set(current_checks.keys()):
        b_err = baseline_checks.get(check_id, {}).get("ERROR", 0)
        c_err = current_checks.get(check_id, {}).get("ERROR", 0)
        if c_err > b_err:
...
def print_verification_stats(records: list[dict]) -> None:
    """Print compact verification stats (events, placements)."""
    total = len(records)
    if total == 0:
        print("Stats: 0 events, 0 placements")
        return
    total_placements = sum(
        len(json.loads(rec.get("placements_json", "[]")))
        for rec in records
    )
    avg = total_placements / total
    print(f"Stats: {total} events, {total_placements} placements (avg {avg:.1f}/event)")
...
def main():
    ...
    print(f"Output: {players_path}")

    # Key stats (one line)
    extras = []
    if overrides and (overrides_applied or overrides_excluded):
        extras.append(f"overrides applied={overrides_applied}, excluded={overrides_excluded}")
    if removed_duplicates:
        extras.append(f"duplicates removed={len(removed_duplicates)}")
    if junk_removed:
        extras.append(f"junk removed={len(junk_removed)}")
    print_verification_stats(canonical)
    if extras:
        print("  " + ", ".join(extras))

    # Run QC checks (compact: one line + regression warning)
    if USE_MASTER_QC:
        qc_summary, qc_issues = run_qc_for_stage("stage2", canonical, out_dir=out_dir)
        # Master QC already wrote stage2_qc_summary.json and stage2_qc_issues.jsonl
        err, warn = qc_summary.get("total_errors", 0), qc_summary.get("total_warnings", 0)
        print(f"QC: {err} errors, {warn} warnings (details: out/stage2_qc_*.json[l])")
        baseline = load_baseline_master(data_dir, "stage2")
        if baseline:
            regressions = _qc_regressions(qc_summary, baseline)
            if regressions:
                print(f"WARNING: QC regressions in: {regressions}")
        if args.save_baseline:
            save_baseline_master(qc_summary, data_dir, "stage2")
    else:
        qc_summary, qc_issues = run_qc(canonical)
        _write_qc_outputs_quiet(qc_summary, qc_issues, out_dir)
        err, warn = qc_summary.get("total_errors", 0), qc_summary.get("total_warnings", 0)
        print(f"QC: {err} errors, {warn} warnings (details: out/stage2_qc_*.json[l])")
