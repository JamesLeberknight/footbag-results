...
def main() -> int:
    ...
    # Person_Stats: use all placements (per) so counts match Placements_ByPerson; gate-3 filtered count remains in Persons_Truth.
    ...
    # Add career columns to Persons_Truth
    # total_placements = all rows in Placements_ByPerson (so "Person Sheets" show full count)
    # total_placements_gate3 = only complete/mostly_complete divisions (for coverage analysis)
    _pbp = placements_by_person_df
    _total_counts = {}
    if "person_id" in _pbp.columns:
        pid = _pbp["person_id"].fillna("").astype(str).str.strip()
        for p in pid:
            if p:
                _total_counts[p] = _total_counts.get(p, 0) + 1
    if "team_person_key" in _pbp.columns:
        for tk in _pbp["team_person_key"].fillna("").astype(str).str.strip():
            if tk and "|" in tk:
                for part in tk.replace("|?", "").split("|"):
                    part = part.strip()
                    if part:
                        _total_counts[part] = _total_counts.get(part, 0) + 1
    _total_df = pd.DataFrame([
        {"effective_person_id": k, "total_placements": v} for k, v in _total_counts.items()
    ])
    _career = (
    ...
    if not _total_df.empty:
        persons_truth_display = persons_truth_display.merge(
            _total_df, on="effective_person_id", how="left"
        )
    persons_truth_display["total_placements"] = (
        persons_truth_display["total_placements"].fillna(0).astype(int)
    )
